...
class Rayonix:
    ...
    def getDiffractogram(self, **kwargs):
        ...
        if fileOutput != None:
            np.savetxt(fileOutput, zip(tthMean, IInt),
                       fmt="%.6e", header="tth\tI")
    ...
    def getCorrectedDiffractogram(self, **kwargs):
        ...
        if fileOutput != None:
            np.savetxt(fileOutput, zip(tth, I, IBase, bck),
    ...
    def normalizeSetDiffractograms(self, rootFilepath, n, **kwargs):
        ...
        if save:
            ...
            np.savetxt(fileOutput, zip(tth, I, IBase, hst), fmt=[
                       "%.6e", "%.6e", "%.6e", "%d"], header="tth\tI\tIBase\thst")
            print("\n'{}' successfully created!".format(fileOutput))
            ...
            for i in range(1, n+1):
                ...
                criteria = (hst == i)
                np.savetxt(fileOutput, zip(
                    tth[criteria], I[criteria], IBase[criteria]), fmt="%.6e", header="tth\tI\tIBase")
                print("\n'{}' successfully created!".format(fileOutput))
